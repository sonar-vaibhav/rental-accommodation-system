...
<body id="page-top" data-bs-spy="scroll" data-bs-target="#mainNav" data-bs-offset="57">
      ...
      <div class="collapse navbar-collapse" id="navbarResponsive">
        <ul class="navbar-nav ms-auto">
          ...
          <li class="nav-item text-capitalize" style="color: rgb(255,255,255);"><a class="nav-link text-capitalize"
              href="login.html" style="color: rgb(255,255,255);margin-top: 13px;">Login&nbsp;</a></li>
        </ul>
      </div>
    </div>
  </nav>
    ...
    <div class="row mb-5">
      <form method="POST">
        <div class="col-10 text-center mx-auto">
          <h2 style="font-size: 24px;margin-bottom: 18px;"><strong><em>Filter the Rooms that fit's you . .
                .</em></strong></h2>
          <div>
            <div class="container text-center">
              <div class="row">
                <div class="col-sm-6 col-md-3">
                  <p class="filter-para">Rent</p>
                  <div class="form-check d-inline-flex" style="padding-right: 7px;">
                    <input class="form-check-input" type="radio" id="formCheck-1" name="price" value="price_low_to_high"
                      style="margin-right: 7px;">
                    <label class="form-check-label" for="formCheck-1">Low to High</label>
                  </div>
                  <div class="form-check d-inline-flex" style="padding-right: 7px;">
                    <input class="form-check-input" type="radio" id="formCheck-5" name="price" value="price_high_to_low"
                      style="margin-right: 7px;">
                    <label class="form-check-label" for="formCheck-5">High to Low</label>
                  </div>
                </div>
                <div class="col-sm-6 col-md-3">
                  <p class="filter-para">Room Type</p>
                  <div class="form-check d-inline-flex" style="margin-left: 13px;">
                    <input class="form-check-input" type="radio" id="formCheck-2" name="type" value="type_shared"
                      style="margin-right: 5px;">
                    <label class="form-check-label" for="formCheck-2">Shared</label>
                  </div>
                  <div class="form-check d-inline-flex" style="margin-left: 13px;">
                    <input class="form-check-input" type="radio" id="formCheck-7" name="type" value="type_personal"
                      style="margin-right: 5px;">
                    <label class="form-check-label" for="formCheck-7">Personal</label>
                  </div>
                </div>
                <div class="col-sm-6 col-md-3">
                  <p class="filter-para">Gender</p>
                  <div class="form-check d-inline-flex" style="margin-left: 13px;">
                    <input class="form-check-input" type="radio" id="formCheck-7" name="gender" value="male"
                      style="margin-right: 5px;">
                    <label class="form-check-label" for="formCheck-7">Male</label>
                  </div>
                  <div class="form-check d-inline-flex" style="margin-left: 13px;">
                    <input class="form-check-input" type="radio" id="formCheck-7" name="gender" value="fe-male"
                      style="margin-right: 5px;">
                    <label class="form-check-label" for="formCheck-7">Fe-Male</label>
                  </div>
                </div>
                <div class="col-sm-6 col-md-3">
                  <p class="filter-para">Rating</p>
                  <div class="form-check d-inline-flex" style="margin-left: 13px;">
                    <input class="form-check-input" type="radio" id="formCheck-3" name="high_rated"
                      style="margin-right: 5px;">
                    <label class="form-check-label" for="formCheck-3">High to Low</label>
                  </div>
                </div>
              </div>
            </div>
            <input class="form-check-input" type="radio" id="formCheck-1" name="def" checked hidden
              style="margin-right: 7px;">

          </div>
          <button class="btn btn-success" type="submit" style="margin:10px; font-size: smaller;">Submit</button>
          <button class="btn btn-danger" type="reset" style="margin:10px; font-size: smaller;">Reset</button>

          <button type="button" class="btn btn-warning" style="margin:10px; font-size: smaller;" data-bs-toggle="modal"
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h1 class="modal-title fs-5" id="exampleModalLabel">Search Rooms by Location</h1>
              ...
              <div class="table-responsive table" id="dataTable-1" role="grid" aria-describedby="dataTable_info">
                <table id="dataTable" style="width:100%;">
                  ...
                  <tbody id="searchResult">
                    ...
                    while ($roomRow = $result2->fetch_assoc()) {
                      echo "
                        <tr>
                        <td>
                          <a class='btn btn-warning text-capitalize d-inline-flex float-start' role='button'
                          href='room.php?room_id=$roomRow[room_id]' name='see_room'>
                          View Details
                          </a>
                        </td>
                        <td>$roomRow[room_rent]</td>
                        <td>$roomRow[room_status]</td>
                        <td>$roomRow[room_type]</td>
                        <td>$roomRow[room_add]</td>
                    </tr>
                ";
                    }
                    ?>
              ...
              <script>
                $(document).ready(function () {
                  ...
                  $('#addressSearch').on('input', function () {
                    ...
                    $.ajax({
                      type: 'POST',
                      url: 'search_by_address.php', // Create this PHP file for address search
  ...
  <div class="row gy-4 row-cols-1 row-cols-md-2 row-cols-xl-3"
    style="margin-top: -29px; margin-left:20px; margin-right:20px;" id="filteredRoom">
    <?php
    include 'connection.php';
    // Function to fetch and filter room data from the database
    function fetchFilteredRooms($conn, $filterOptions)
    {
      ...
      if (isset($filterOptions['def'])) {
        // Default condition, no need to change $sql
        $conditionsApplied = true;
      } elseif (isset($filterOptions['price'])) {
        if ($filterOptions['price'] == 'price_low_to_high') {
          $sql .= " ORDER BY room_rent ASC";
          $conditionsApplied = true;
        } elseif ($filterOptions['price'] == 'price_high_to_low') {
          $sql .= " ORDER BY room_rent DESC";
          $conditionsApplied = true;
        }
      } elseif (isset($filterOptions['high_rated'])) {
        $sql .= " ORDER BY (SELECT AVG(rr_rating) FROM room_reviews WHERE room_reviews.room_id = rooms.room_id) DESC";
        $conditionsApplied = true;
      }

      if (isset($filterOptions['available_only']) && !$conditionsApplied) {
        $sql .= " WHERE room_status = 'Available'";
      }

      if (isset($filterOptions['type'])) {
        $sql = $filterOptions['type'] == 'type_shared' ? "SELECT * FROM rooms WHERE room_type = 'Shared'" : "SELECT * FROM rooms WHERE room_type = 'Personal'";
      }
      if (isset($filterOptions['gender'])) {
        $sql = $filterOptions['gender'] == 'male' ? "SELECT * FROM rooms WHERE room_gender = 'Male'" : "SELECT * FROM rooms WHERE room_gender = 'Fe-Male'";
      }
      ...
      if ($result) {
        ...
        $filteredRooms = mysqli_fetch_all($result, MYSQLI_ASSOC);
        // Free the result set
      ...
      // Return the filtered rooms data
      return $filteredRooms;
    }

    // Example usage: Fetch filtered room data based on user selection
    $filterOptions = [
      'price' => isset($_POST['price']) ? $_POST['price'] : null,
      'available_only' => isset($_POST['available_only']),
      'high_rated' => isset($_POST['high_rated']),
      'type' => isset($_POST['type']) ? $_POST['type'] : null,
      'gender' => isset($_POST['gender']) ? $_POST['gender'] : null,
    ];

    $filteredRoomData = fetchFilteredRooms($conn, $filterOptions);
    // Display filtered room data in the HTML card structure
    foreach ($filteredRoomData as $room):
      ?>
        ...
        <div class="card" style="border-radius: 19px;">
          ...
          <div class="card-body p-4" style="box-shadow: 0px 0px;">
                ...
                <?php foreach ($imageUrls as $index => $imageUrl): ?>
                  <div class="carousel-item <?= ($index === 0) ? 'active' : ''; ?>">
                    <img height="300px" width="450px" class="w-100 d-block"
                      src="Users/<?php echo substr($imageUrl['img_path'], 3); ?>" alt="Room Image">
                  </div>
                <?php endforeach; ?>
              </div>
            ...
            <div style="margin:10px; height:44px;">
              <a class="btn btn-primary d-inline-flex float-end" onclick="noAccess()"
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" viewBox="0 0 16 16"
                  class="bi bi-heart" style="color: var(--bs-white);">
                  <path
              ...
              <button class="btn btn-primary d-inline-flex float-end" onclick="noAccess()" data-bs-toggle="modal"
            ...
            <div style="height: 36px;margin: 3px;">
              ...
              <p class="d-inline-flex float-end" style="margin-top: 10px;">
                <?php echo $room['room_rent']; ?> / Month
              </p>
              <p class="d-inline-flex float-end" style="margin-right: 7px;margin-top: 10px;">Rs.</p>
            </div>
            ...
            <div style="display: inline-flex; margin-left: -115px;">
              ...
              <p class="d-inline-flex float-start"
                style="margin: 0;margin-left: 0;margin-top: 0;margin-right: 3px;font-weight: bold;">
                <img src="Users/<?php echo substr($ownerPhoto, 3); ?>" alt=""
            ...
            <div style="height: 43px;margin: 0;margin-left: 0;margin-top: 23px;padding: 1px;">
              <a class="btn btn-warning text-capitalize d-inline-flex float-start" role="button" target="_blank"
                href="room.php?room_id=<?php echo $room['room_id']; ?>" name="see_room">
                View Details
              </a>

              <button class="btn btn-success text-capitalize d-inline-flex float-end" onclick="noAccess()" type="submit">
                Request Call 📞&nbsp;
              </button>
            </div>
          </div>
  ...
  if (isset($_GET['already_added'])) {
    echo "
            <script>alert('Room is Already in Favorites');</script>
            ";
  }

  if (isset($_GET['success'])) {
    echo "
            <script>alert('Room is Added in Favorites');</script>
            ";
  }
  if (isset($_GET['error'])) {
    echo "
            <script>alert('Room is not Added in Favorites');</script>
            ";
  }
  if (isset($_GET['invalid_request'])) {
    echo "
            <script>alert('Invalid Request');</script>
            ";
  }
  ?>

  <div class="row gy-4 row-cols-1 row-cols-md-2 row-cols-xl-3" style="display:none;margin-top: 0px;" id="ogRoom">
        ...
        <div class="card" style="border-radius: 19px;">
          ...
          <div class="card-body p-4" style="box-shadow: 0px 0px;">
            ...
            <div style="height: 43px;margin: 0;margin-left: 0;margin-top: 23px;padding: 1px;">
              ...
              <button class="btn btn-success text-capitalize d-inline-flex float-end" type="button">Request
                Call 📞&nbsp;</button>
            </div>

            <!-- Add more details about the room -->
            <!-- ... -->
          </div>
        </div>
      </div>
